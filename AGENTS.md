...
Zip archives are indexed separately in `zips`; `files.md5` is the per-file checksum and `files.zip_id` is the archive link when a row belongs to a zip.

Persistent scan-folder cache: `run_scan_folders` rows survive run completion and seed later full runs in the same profile DB; scan-only runs and explicit `ScanPaths` rescans bypass that cache.
