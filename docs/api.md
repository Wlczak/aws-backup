...
POST   /api/auth/setup              one-time {password} bootstrap; clears cookies and requires login
...
Every `/api/*` route except the auth endpoints and browser-log ingest requires a valid signed auth cookie. Creating the first password does not authenticate the browser: `/api/auth/setup` clears stale cookies and the user must prove the new password through `/api/auth/login`. While `setup_required=true`, authenticated access is further restricted to settings, folder browsing, source/S3 tests, and `POST /api/setup/complete`; operational routes return `428 Precondition Required`. The completion endpoint independently validates the full config and repeats both connectivity tests before persisting `setup_completed=true`.
