...
- the CLI's `serve` shutdown path retries `Server.Shutdown` without its 10s deadline if the bounded wait times out, so `app.close()` does not tear down DB/storage while an engine run or post-run DB sync is still draining
...
- Restore-trigger and inventory-sync are now background jobs instead of request-bound handlers. `/api/restore/trigger` returns a `restore_job_id` immediately and the job keeps running under a server-scoped context even if the browser disconnects. `/api/status` exposes `restore_job_current` / `restore_job_last`, and `GET /api/restore/jobs/{id}` lets the UI recover a job after refresh. Inventory sync emits `restore_manifest_*` SSE events while it locates and parses the latest manifest, then switches to the existing `restore_scan_*` events for the HEAD reconciliation phase.
- Dashboard restore-maintenance controls use `/api/status.restore_job_current` as their concurrency guard. SQS drains and full restore scans stay disabled while a persisted restore-trigger or inventory-sync job is running, including after a page refresh.
- The Restore page also keeps "Sync from inventory" disabled for the complete persisted restore-job lifecycle, including the scan phase after manifest progress has cleared.
- The Restore page rebuilds terminal job visibility from `/api/status.restore_job_last`; failed and cancelled retrieval/inventory jobs remain visible after a browser reload with their job ID, phase, and server error.
