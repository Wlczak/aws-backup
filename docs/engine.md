...
- `RestoreToDir` downloads matching uploaded/zipped rows into an operator-selected absolute directory, extracts zip members selectively, and verifies each restored file against the row's `md5` unless `SkipChecksum` is set. The API handler wires progress into `restore_download_*` SSE events so the Restore page can show live download/verify progress.
- `/api/restore/download/estimate` uses the same DB path matching as `RestoreToDir` to estimate the number of S3 objects and the indexed bytes that the Download tab will fetch, then prices GET requests plus outbound egress.
