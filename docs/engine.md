...
- `POST /api/restore/to-dir` calls `engine.RestoreToDir` with the configured temp directory, downloads matching S3 objects into a local destination, unzips zip archives inline, and verifies each written file against the stored MD5 unless the caller explicitly disables verification. Rows that are still in `restore_status='in_progress'` are surfaced back to the UI as blocked paths instead of being downloaded.
