...
## Dashboard Full Download

`POST /api/download/full` is a separate mirror job, not part of `Engine.RunWithID`.
...
- `downloadWg` waits on the dashboard full-download goroutine during `Server.Shutdown`
...
- The full-download job tracks the missing-set object count plus estimated GET/egress/total cost in the live job summary so the dashboard can surface the price alongside scan and progress state.
