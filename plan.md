...
| —    | ea82784 | `S3.MultipartThreshold` (bytes; 0 = default 5 GiB) is now a setting; storage swaps the const for a per-instance field, and the Settings UI exposes a number input. Lets operators get parallel-part throughput on medium-sized objects without waiting for the 5 GiB ceiling |
| #127 | (pending) | Resume cached tmp on upload retry: stable `ind-{fileID}` tmp name; new `tryReuseTmp` helper checks size + mtime + re-hashes locally; `uploadIndividual` skips `copyAndHash` on a usable cached tmp; tmp removed only on success / dedup-skip / poisoned partial; new `sweepOrphanTmps` at upload-phase start removes `ind-N` files whose ID isn't in the current pending list. `PendingFile` carries `MTime` from `db.File` |
| #133 | 5aa8908 | Pre-flight `skipIfMatches` dedup before each `Storage.Put` (individual files, zip body, zip index sidecar): HEAD destination with `ChecksumMode=ENABLED`, compare against locally-computed SHA256, mark uploaded without re-uploading on match. `copyAndHash` now returns sha256 alongside md5; new `sha256File` helper; `HeadResult.ChecksumSHA256` populated by S3 + MemStorage. Prevents redundant new versions on versioned buckets without DB schema changes |
| #136 | 9004b24 | Dashboard `itemList` comparator pins active uploads above active copies (phase tiebreaker before `updatedAt`) so the in-flight S3 upload bar stops being buried by chatty `copy_progress` events |
