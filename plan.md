...
| #137 | (pending) | `source.Scan` accepts an optional `ProgressFn` invoked from the flusher goroutine after each successful batch upsert; engine emits `EventScanProgress` with `{seen, new, changed}`; Dashboard renders an indeterminate sliding bar + live counters under the existing "Live progress" card while `scanActive` |
