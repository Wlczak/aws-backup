...
| —    | (pending) | `S3.MultipartThreshold` (bytes; 0 = default 5 GiB) is now a setting; storage swaps the const for a per-instance field, and the Settings UI exposes a number input. Lets operators get parallel-part throughput on medium-sized objects without waiting for the 5 GiB ceiling |
